...
$(document).ready(function() {
    $('#commentForm').on('submit', function(e) {
        ...
        $.ajax({
            url: '{{ route("comments.store") }}',  // Pastikan rute benar
            method: 'POST',
            data: {
                _token: '{{ csrf_token() }}',
                post_id: postId,
            ...
            success: function(response) {
                // Cek jika respons JSON berhasil
                if (response.success) {
                    // HTML untuk menambahkan komentar baru
                    var commentHtml = `
                        <div class="card mb-3">
                            <div class="card-body">
                    ...
                    `;
                    // Tambahkan komentar ke dalam container komentar
                    $('#commentsContainer').prepend(commentHtml);

                    // Kosongkan input komentar setelah sukses
                    $('#commentContent').val('');
                } else {
