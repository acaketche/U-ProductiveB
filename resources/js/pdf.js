...
var createPDFThumbnails = function(){
    ...
    function renderThumbnails() {
        ...
        nodesArray.forEach(function(element) {
            ...
            var path = element.getAttribute('data-pdf-thumbnail-file');
            var imgWidth = element.getAttribute('data-pdf-thumbnail-width');
            var imgHeight = element.getAttribute('data-pdf-thumbnail-height');

            pdfjsLib.getDocument({url: path, worker: worker}).promise.then(function (pdf) {
                pdf.getPage(1).then(function (page) {
                    ...
                    console.log("pdfThumbnails error: could not open page 1 of document " + path + ". Not a pdf ?");
                });
            }).catch(function() {
                console.log("pdfThumbnails error: could not find or open document " + path + ". Not a pdf ?");
            });
        });
    }
};
